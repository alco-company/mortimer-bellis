module EmployeesHelper
  ...
  def link_to_team_employees_status(team)
    return unless User.by_tenant().count > 0
    link_to t("teams.list.user_status"), pos_employees_url(t: team, api_key: first_employee_token), class: "inline-flex items-center rounded-md bg-gray-50 px-2 py-1 text-xs font-medium text-gray-600 ring-1 ring-inset ring-gray-500/10", target: "_blank", data: { turbo_frame: "_top", turbo_stream: true }
  end
