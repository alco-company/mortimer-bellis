module Authorize
  ...
  def authorize
    ...
    when "editor", "settings", "provided_services"; Current.user&.admin? ? true : false
    when "projects", "teams"; Current.get_tenant.license == "free" ? false : true
    when "users/invitations"; %(trial ambassador essential pro).include?(Current.get_tenant.license) && Current.user&.admin? ? true : false
    when "users"; %(trial ambassador essential pro).include?(Current.get_tenant.license) && Current.user&.admin? ? true : false
