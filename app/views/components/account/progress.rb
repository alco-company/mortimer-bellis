        ...
        if Current.get_user.tasks.any?
          ...
          nav(class: "flex h-35 overflow-y-auto scrollbar-hide", aria_label: "Progress") do
            ol(role: "list", class: "text-slate-700 space-y-2") do
              Task.by_tenant.tasked_for_the(Current.get_user).order(completed_at: :asc).each do |task|
                render DashboardTask.new task: task, show_options: false, menu: true
              end
            end
          end
        else
          h3(class: "text-lg font-bold text-sky-700 mb-2") { t("profile.no_tasks") }
          p(class: "text-slate-500") { t("profile.no_tasks_message") }
        end
      end
    end
  end
