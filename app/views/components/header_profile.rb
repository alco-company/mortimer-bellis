class HeaderProfile < ApplicationComponent
  ...
  #   <div>
  #     <h3 class="text-lg font-bold text-slate-700">God eftermiddag, Andreas</h3>
  ...
  def view_template
    ...
    div(class: "flex items-center") do
      if Current.user.mugshot.attached?
        image_tag(Current.user.mugshot, alt: "user avatar", class: "hidden h-16 w-18 rounded-full sm:block")
      else
        image_tag("icons8-customer-64.png", alt: "user avatar", class: "hidden h-16 w-18 rounded-full sm:block")
      end
      div(class: "flex flex-col ml-4") do
        h3(class: "text-lg font-bold text-sky-700") { salutation }
        tenant_presentation
  ...
  def tenant_presentation
    dl(class: "mt-6 flex flex-col -ml-1 sm:mt-1 sm:flex-row sm:flex-wrap") do
      dt(class: "sr-only") { "Company" }
      dd(class: "flex items-center text-sm font-medium capitalize text-gray-500 sm:mr-6") do
        svg(
          class: "mr-1.5 h-5 w-5 flex-shrink-0 text-gray-400",
          viewbox: "0 0 20 20",
          fill: "currentColor",
