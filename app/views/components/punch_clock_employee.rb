class PunchClockEmployee < PunchClockBase
      ...
      div(class: %(w-full fixed bottom-[39px] z-40 bg-slate-100 opaque-5)) do
        div(class: "max-w-full lg:max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 mb-2") do
          div(class: "border-gray-200 py-2 text-gray-400 justify-items-stretch flex flex-row-reverse gap-2") do
            render PunchClockButtons.new user: user, tab: tab, url: pos_user_url(api_key: user.access_token), edit: edit
          end
        end
  ...
  def punch_more
    div(class: "flex flex-row flex-wrap text-xs hidden", data: { pos_user_target: "evenMoreOptions" }) do
      extra_button "bg-red-500", t(".child_sick"), :child_sick
      extra_button "bg-red-500", t(".nursing_sick"), :nursing_sick
      extra_button "bg-red-500", t(".p56_sick"), :p56_sick
      extra_button "bg-red-500", t(".lost_work_sick"), :lost_work_sick
      extra_button "bg-blue-500", t(".rr_free"), :rr_free
      extra_button "bg-blue-500", t(".senior_free"), :senior_free
      extra_button "bg-blue-500", t(".unpaid_free"), :unpaid_free
      extra_button "bg-blue-500", t(".maternity_free"), :maternity_free
      extra_button "bg-blue-500", t(".leave_free"), :leave_free
    end
    div(class: "flex flex-row flex-wrap text-xs hidden", data: { pos_user_target: "moreOptions" }) do
      button_tag(t(".even_more_options"), type: "button", data: { action: "click->pos-user#toggleEvenMoreOptions" }, class: "bg-slate-200 text-white block mx-2 my-2 rounded-md px-3 py-2 font-medium")
      extra_button "bg-blue-500", t(".free"), :free
      extra_button "bg-red-500", t(".iam_sick"), :iam_sick
    end
  end
  ...
  def extra_button(color, text, state)
    div(class: "justify-self-end") do
      ...
      form_with url: pos_user_url(api_key: user.access_token), id: "form_#{ state }", method: :post do
