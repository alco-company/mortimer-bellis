    ...
    if @button_only
      ...
      div(id: "two_factor_field_button", class: %( mt-0 flex-shrink-0)) do
        if Current.user.otp_enabled
          link_to(
            t("devise.second_factor.disable_2fa"),
          ...
          link_to(new_users_otp_url(button_only: true), data: { turbo_stream: true }, class: css) do
            span(class: "text-sm mr-2") { t("devise.second_factor.enable_2fa") }
      ...
      div(id: "two_factor_field", class: %(rounded-md bg-sky-100 px-6 py-5 flex items-start justify-between)) do
        ...
        div(class: "ml-4 mt-0 flex-col") do
          div(class: %(text-sm font-medium text-gray-900)) { t("devise.second_factor.authenticator_app") }
          div(class: %(mt-1 text-sm text-gray-600 flex items-center)) do
            if Current.user.otp_enabled
              div do
                span(class: "block") { t("devise.second_factor.enabled") }
                span(class: "block") { l(Current.user.otp_enabled_at, format: :date) }
                span(class: %(hidden sm:inline)) { Current.user.otp_enabled_at.strftime("%H:%M") }
              end
            else
              div { t("devise.second_factor.disabled") }
            end
            div(id: "two_factor_field_button", class: %(ml-6 mt-0 flex-shrink-0)) do
              if Current.user.otp_enabled
                link_to t("devise.second_factor.disable_2fa"), users_otp_url, method: :delete, data: { turbo_stream: true }, class: "mort-btn-alert"
              else
                link_to t("devise.second_factor.enable_2fa"), new_users_otp_url, data: { turbo_stream: true }, class: "mort-btn-primary"
              end
            end
          end
        end
