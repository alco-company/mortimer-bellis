class PunchCards::Form < ApplicationForm
  def view_template(&)
    ...
    row field(:employee_id).select(Employee.by_account.order(name: :asc).select(:id, :name), prompt: I18n.t(".select_employee"), class: "mort-form-text") unless model.employee
