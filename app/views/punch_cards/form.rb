class PunchCards::Form < ApplicationForm
  def view_template(&)
    view_only field("user.name").input(class: "mort-form-text", value: model.user.name) if model.user
    row field(:user_id).select(User.by_tenant.order(name: :asc).select(:id, :name), prompt: I18n.t(".select_user"), class: "mort-form-text") unless model.user
