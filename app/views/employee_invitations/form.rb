class EmployeeInvitations::Form < ApplicationForm
  def view_template(&)
    div(data: { controller: "employee_invitation" }) do
      row field(:user_id).select(User.by_account.order(name: :asc).select(:id, :name), prompt: I18n.t(".select_user"), class: "mort-form-text").focus
      ...
      row field(:address).input(class: "mort-form-text", placeholder: I18n.t("employee_invitation.address_placeholder"))
