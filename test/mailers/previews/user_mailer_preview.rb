...
class UserMailerPreview < ActionMailer::Preview
  ...
  def confetti_first_punch
    ...
    UserMailer.with(user: @user, name: @name, company: @company, sender: @sender).confetti_first_punch
  end
  ...
  def user_farewell
    ...
    UserMailer.with(user: user, name: name, company: company, sender: sender).user_farewell
  end
  ...
  def last_farewell
    ...
    @company = @user.tenant&.name || "MORTIMER"
    @sender = @user.tenant&.email || "The Mortimer Team"
    UserMailer.with(user: @user, name: @name, company: @company, sender: @sender).last_farewell
  end

  def info_report
    @info = "Some info"
    @msg = "Some message"
    UserMailer.with(info: @info, msg: @msg).info_report(@info, @msg)
  end

  def error_report
    @error = "Some error"
    @klass_method = "some_klass_method"
    UserMailer.with(error: @error, klass_method: @klass_method).error_report(@error, @klass_method)
  end
  ...
  def confirmation_instructions
    ...
    @email = @user.email
    @confirmation_url = "/"
    UserMailer.with(user: @user, email: @email, confirmation_url: @confirmation_url).confirmation_instructions(@user)
  end
  ...
  def invitation_instructions
    ...
    @accept_url = "/"
    UserMailer.with(resource: @resource,
